...
library(MEDIPS)
library(BSgenome)
source("/data/fg2/guerrero/cheetah_methylation/MeDIPS/master_code/dataNames_AciJub.R")

##################### TANIA's NOTE: This step it will be likely unnecessary if ref genome 
...
# Tania G: for my MSc thesis I built the index in the terminal, cuz this one was not working. Then realized it was just missing to direct it to the fasta individual file. 
#So w/that "correction" this one should work just fine 
#ATTENTION: Also this index is for the time being already existing in the data folder so this step should not be necessary unless requested or using a different genome version.
...
# Tania G: I did the mapping as follows without changes. This should be validated in the future. 
for (i in 1:length(seqFiles$sampleName)) {
  if (mapReads) {
...
## Tania G: This one works but I also used the one from Forge BSgenome directly (script forgeBSGenome2_Cheeta.R)
if (buildBSgenome){
